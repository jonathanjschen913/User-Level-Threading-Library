...
main(void)
{
	...
	for (ii = 0; ii < NTHREADS; ii++) {
		...
		assert(ret > 0);
		child[ii] = thread_create((thread_entry_f)hello, msg[ii]);
	...
	for (ii = 0; ii < THREAD_MAX_THREADS - 1; ii++) {
		child[ii] = thread_create((thread_entry_f)fact, (void *)10);
		assert(thread_ret_ok(child[ii]));
	}
	/*
	 * Now we're out of threads. Next create should fail.
	 */
	ret = thread_create((thread_entry_f)fact, (void *)10);
	...
	int flag = set_flag(0);
	ret = thread_create((thread_entry_f)suicide, NULL);
	assert(thread_ret_ok(ret));
...
grand_finale()
{
	...
	printf("while my talented assistant prints \"a1 test done\"\n");
	ret = thread_create((thread_entry_f)finale, NULL);
	assert(thread_ret_ok(ret));
	thread_exit(0);
}
